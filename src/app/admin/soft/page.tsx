...
export default function AdminSoftPage() {
  ...
  const loadEmployees = React.useCallback(async (page: number = 1) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/admin/karyawan?page=${page}`, {
        method: "GET",
        headers: {
      ...
      const mapped: EmployeeRow[] = list.map((item) => ({
        id: String(item.id), // internal key saja
        nik: item.nik ?? "-",
        nama: item.name ?? "-",
  ...
  // ✅ Sekarang detail pakai NIK di URL
