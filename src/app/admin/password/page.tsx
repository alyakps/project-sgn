      ...
      >
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            className={
              newPassword ? (isStrongPassword ? "border-green-500" : "border-red-500") : ""
            }
          />
...
function Rule({ label, ok }: { label: string; ok: boolean }) {
  return (
    <div className={`flex items-center gap-2 ${ok ? "text-emerald-600" : "text-zinc-500"}`}>
      {ok ? <CheckCircle2 className="h-3.5 w-3.5" /> : <XCircle className="h-3.5 w-3.5 text-red-500" />}
      <span>{label}</span>
    </div>
  );
}
